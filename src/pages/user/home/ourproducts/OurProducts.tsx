    ...
    <div className="w-full py-24 lg:px-24 px-4 relative">
      ...
      >
        {imgesData.map((service, index) => (
          <div
            key={index}
            className="relative bg-gray-400 h-[15rem] w-[25rem] rounded-xl flex-shrink-0"
          >
            <img
              src={service.image}
              alt=" prod_image"
            ...
            <div className="bg-white absolute -top-2 left-8 p-2 rounded-xl text-lg px-4">{service.title}</div>
          </div>
        ))}
      </div>
    </div>
